...
class PagerBottomTabStrip extends LinearLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //将导航栏宽度转换成DP,用于计算
        int width_dp = (int) UIUtils.px2dp(mContext, getMeasuredWidth());

        //设置按钮的初始宽度
        if ((mMode & TabLayoutMode.HIDE_TEXT) > 0) {
            int max_w = HIDE_TEXT_MAX_WIDTH * (mTabItems.size() - 1) + HIDE_TEXT_SELECTED_MAX_WIDTH;
            if (width_dp > max_w) {
                int leftPadding = (int) UIUtils.dp2px(mContext, (width_dp - max_w) / 2f);
                setPadding(leftPadding, 0, 0, 0);

                mSelectedWidth = (int) UIUtils.dp2px(mContext, HIDE_TEXT_SELECTED_MAX_WIDTH);
                mDefultWidth = (int) UIUtils.dp2px(mContext, HIDE_TEXT_MAX_WIDTH);
            } else {
                ...
                mDefultWidth = (int) UIUtils.dp2px(mContext, default_width);
                mSelectedWidth = (int) UIUtils.dp2px(mContext, selected_width);
            }
            ...
            if (w > DEFAULT_MAX_WIDTH) {
                int leftPadding = (int) UIUtils.dp2px(mContext, (w - DEFAULT_MAX_WIDTH) * mTabItems.size() / 2f);
                setPadding(leftPadding, 0, 0, 0);
                mSelectedWidth = mDefultWidth = (int) UIUtils.dp2px(mContext, DEFAULT_MAX_WIDTH);
            } else {
                mSelectedWidth = mDefultWidth = (int) UIUtils.dp2px(mContext, w);
            }
    ...
    protected void setSetectManually(int index) {
        ...
        float y = UIUtils.dp2px(mContext, 28);
    ...
    class builder implements TabStripBuild {
        ...
        @Override
        public TabStripBuild addTabItem(TabItemBuilder tabItemBuilder) {
    ...
    private void addItem(Drawable drawable, Drawable selectedDrawable, String text) {
        int selectedColor = 0xff0000; //UIUtils.getAttrColor(mContext, R.attr.colorAccent);
