...
import com.hackathon.exchange.utils.UIUtils;
...
//            TabItem.this.setBackgroundResource(UIUtils.getResourceId(mContext, R.attr.selectableItemBackgroundBorderless));
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(getMeasuredWidth(), (int)UIUtils.dp2px(mContext, 56));
    ...
    private void drawText(Canvas canvas, float n) {
        ...
        textPaint.setTextSize(UIUtils.sp2px(mContext, TEXTSIZE_DEFAULT + (TEXTSIZE_SELECTED - TEXTSIZE_DEFAULT) * n));
        ...
        float y = getMeasuredHeight() - UIUtils.dp2px(mContext, 10) - textBound.height() / 2f - fontMetrics.descent + (fontMetrics.descent - fontMetrics.ascent) / 2;
    ...
    private void drawIcon(Canvas canvas, float n) {
        int width = (int) UIUtils.dp2px(mContext, WIDTH_ICON);
        ...
        float left = getMeasuredWidth() / 2f - UIUtils.dp2px(mContext, 12);

        float top;
        if ((mMode & TabLayoutMode.HIDE_TEXT) > 0) {
            top = UIUtils.dp2px(mContext, 16 - 10 * n);
        } else {
            top = UIUtils.dp2px(mContext, 8 - 2 * n);
        }
    ...
    private void drawMessages(Canvas canvas, float n) {
        if (mMessageNumber > 0) {
            ...
            int width = (int) UIUtils.dp2px(mContext, 20);
            Bitmap bitmap = Bitmap.createBitmap(width, width, Bitmap.Config.ARGB_8888);
            ...
            numberPaint.setTextSize(UIUtils.dp2px(mContext, textSize));
            ...
            float left = getMeasuredWidth() / 2f + UIUtils.dp2px(mContext, 10);

            float top;
            if ((mMode & TabLayoutMode.HIDE_TEXT) > 0) {
                top = UIUtils.dp2px(mContext, 12 - 6 * n);
            } else {
                top = UIUtils.dp2px(mContext, 6);
            }
            canvas.drawBitmap(bitmap, left, top, null);
    ...
    private void drawOval(Canvas canvas, float n) {
        if (hasMessages && mMessageNumber <= 0) {
            ...
            float width = UIUtils.dp2px(getContext(), 10);
...
//            if(mColorSelected == 0)
//            {
//                mColorSelected = UIUtils.getAttrColor(mContext,R.attr.colorAccent);
//            }
        ...
        @Override
        public TabItemBuild setText(String text) {
            mText = text;
        ...
        @Override
        public TabItemBuild setTag(Object object) {
        ...
        private Bitmap getICON(Drawable drawable) {
            ...
            Bitmap oldbmp = UIUtils.drawableToBitmap(drawable);
            Matrix matrix = new Matrix();
            float scaleWidth = (UIUtils.dp2px(mContext, 24) / width);
            float scaleHeight = (UIUtils.dp2px(mContext, 24) / height);
